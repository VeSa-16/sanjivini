...
export default function HistoryPage() {
  ...
  const [financials, setFinancials] = useState(getFinancials());
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
      <div>
        <div className="mb-4 flex gap-2 rounded-2xl bg-[#edf4e9] p-1 shadow-inner max-w-fit">
          <button 
            onClick={() => setActiveTab("timeline")}
            className={`rounded-xl px-5 py-2.5 text-sm font-bold transition ${activeTab === "timeline" ? "bg-white text-[#173f2c] shadow-sm" : "text-[#69746c] hover:bg-white/50"}`}
          >
            Timeline
          </button>
          <button 
            onClick={() => setActiveTab("ledger")}
            className={`rounded-xl px-5 py-2.5 text-sm font-bold transition ${activeTab === "ledger" ? "bg-white text-[#173f2c] shadow-sm" : "text-[#69746c] hover:bg-white/50"}`}
          >
            Farm Ledger
          </button>
        </div>
        ...
        {activeTab === "timeline" ? (
          <Card className="p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {(logs.length > 0 || financials.length > 0) && <Button variant="danger" onClick={clear}>Clear demo history</Button>}
            </div>
      ...
      <EndOfCropSummary crop={crop} logs={logs} financials={financials} />
    </div>
  );
}
